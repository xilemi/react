...
import { addRoleApi, addTypeAnnoApi, addTypeCartApi, delRoleApi, delTypeAnnoApi, delTypeCartApi, getRoleListApi, listTypeAnnoApi, listTypeCartApi, updataRoleApi, updateCartTypeApi, updateTypeAnnoApi } from '@/api/pro';
...
const AnnoType = () => {
  ...
  const addConfirm = async (val) => {
    let res = await addTypeCartApi(val)
    if (res.code == 200) {
  ...
  const updataConfirm = async (val) => {
    let res = await updateCartTypeApi({ ...val, _id: modalRef.current.id })
    if (res.code == 200) {
      actionRef.current?.reload()
      updataRoles()
    }
        ...
        <a
          ...
          onClick={() => {
            confirmMessage("是否删除", async () => {
              let res = await delTypeCartApi({ _id: record._id })
              if (res.code == 200) {
                // 存mobx 先删除前端数据
                actionRef.current?.reload()
              }
  ...
  return (
    <div>
      <MyModalForm ref={modalRef} title={flag ? "添加车辆类型" : '修改车辆类型'} confirm={flag ? addConfirm : updataConfirm}>
        <ProFormText
          name="label"
          label="车辆类型"
        ...
        <ProFormText
          name="value"
          label="车辆等级"
          rules={[{ pattern: /\d/, message: '车辆等级只能是数字' }, { required: true, message: "请填写" }]}
        />
        <ProFormColorPicker
          name="color"
          label="车辆属性"
      ...
      <ProTable<GithubIssueItem>
        ...
        request={async (params = {}, sort, filter) => {
          ...
          let res = await listTypeCartApi({ ...params, sort: sort.value == "ascend" ? 1 : -1 })
          if (res.code == 200) {
