...
import { addRoleApi, addTypeAnnoApi, delRoleApi, delTypeAnnoApi, getRoleListApi, listTypeAnnoApi, updataRoleApi, updateTypeAnnoApi } from '@/api/pro';
...
const AnnoType = () => {
  ...
  const addConfirm = async (val) => {
    let res = await addTypeAnnoApi(val)
    if (res.code == 200) {
  ...
  const updataConfirm = async (val) => {
    let res = await updateTypeAnnoApi({ ...val,_id:modalRef.current.id })
    if (res.code == 200) {
      actionRef.current?.reload()
      updataRoles()
    }
        ...
        <a
          ...
          onClick={() => {
            confirmMessage("是否删除", async () => {
              let res = await delTypeAnnoApi({ _id: record._id })
              if (res.code == 200) {
                // 存mobx 先删除前端数据
                actionRef.current?.reload()
              }
    ...
    <div>
      ...
      <ProTable<GithubIssueItem>
        ...
        //  sort 得到的是个对象,value,需要后端接受
          let res = await listTypeAnnoApi({...params,sort:sort.value=="ascend"?1:-1})
          if (res.code == 200) {
