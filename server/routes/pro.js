...
const storage = multer.diskStorage({
    ...
    filename: function (req, file, cb) {
        cb(null, "xile" + '-' + Date.now() + '-' + file.originalname)
    }
})
...
// 查询车辆类型
router.post("/cart/type/list", (req, res) => {
    var query = {}
    var body = req.body;
    if (body.label) {
        query.label = new RegExp(body.label)
...
// 删除车辆类型
router.post("/cart/type/delete", (req, res) => {
    var body = req.body
    decodeToken(req, res, ({ phone }) => {
        removeDataFromTable({
            model: ProCartTypeModel,
...
// 修改 车辆
router.post("/cart/type/update", (req, res) => {
    var body = req.body
    decodeToken(req, res, ({ phone }) => {
        updateDataFromTable({
            model: ProCartTypeModel,
